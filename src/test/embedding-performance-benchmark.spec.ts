...
// 각 서비스별로 실제 벡터를 생성하므로 고정된 testEmbeddings 제거
...
describe('임베딩 서비스 성능 벤치마크', () => {
  ...
  describe('MiniLM 임베딩 서비스', () => {
    ...
    it('유사도 검색 성능', async () => {
      ...
      // MiniLM용 실제 벡터 생성
      const minilmEmbeddings: EmbeddingData[] = [];
      for (let i = 0; i < testTexts.length; i++) {
        const result = await minilmService.generateEmbedding(testTexts[i]);
        if (result) {
          minilmEmbeddings.push({
            id: `minilm-test-${i}`,
            content: testTexts[i],
      ...
      const result = await benchmark.measureOperation(
        'minilm',
        'similarity-search',
        () => minilmService.searchSimilar(query, minilmEmbeddings, 5, 0.7)
      );
  ...
  describe('TF-IDF 임베딩 서비스', () => {
    ...
    it('유사도 검색 성능', async () => {
      ...
      // TF-IDF용 실제 벡터 생성
      const tfidfEmbeddings: EmbeddingData[] = [];
      for (let i = 0; i < testTexts.length; i++) {
        const result = await tfidfService.generateEmbedding(testTexts[i]);
        if (result) {
          tfidfEmbeddings.push({
            id: `tfidf-test-${i}`,
            content: testTexts[i],
            embedding: result.embedding
          });
      ...
      const result = await benchmark.measureOperation(
        'tfidf',
        'similarity-search',
        () => tfidfService.searchSimilar(query, tfidfEmbeddings, 5, 0.7)
      );
  ...
  describe('통합 임베딩 서비스', () => {
    it('MiniLM 제공자로 임베딩 생성 성능', async () => {
      ...
      expect(result).toBeDefined();
      // MiniLM은 384차원, TF-IDF는 512차원 - 실제 사용되는 제공자에 따라 다름
    ...
    it('유사도 검색 성능', async () => {
      ...
      // 통합 서비스용 실제 벡터 생성 (실제 사용되는 제공자에 맞는 차원)
      const unifiedEmbeddings: EmbeddingData[] = [];
      for (let i = 0; i < testTexts.length; i++) {
        const result = await unifiedService.generateEmbedding(testTexts[i]);
        if (result) {
          unifiedEmbeddings.push({
            id: `unified-test-${i}`,
            content: testTexts[i],
            embedding: result.embedding
          });
      ...
      const result = await benchmark.measureOperation(
        'unified-minilm',
        'similarity-search',
        () => unifiedService.searchSimilar(query, unifiedEmbeddings, 5, 0.7)
      );
