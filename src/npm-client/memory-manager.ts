...
export class MemoryManager {
   ...
   * 기억 생성
   */
  async create(params: CreateMemoryParams): Promise<MemoryItem> {
   ...
   * 기억 업데이트
   */
  async update(id: string, params: UpdateMemoryParams): Promise<MemoryItem> {
   ...
   * 태그로 기억 검색
   */
  async searchByTags(tags: string[], limit?: number): Promise<SearchResult> {
   ...
   * 타입별 기억 검색
   */
  async searchByType(type: MemoryType, limit?: number): Promise<SearchResult> {
    return await this.search('*', {
      filters: { type: [type] },
      limit: limit || undefined
    });
   ...
   * 프로젝트별 기억 검색
   */
  async searchByProject(projectId: string, limit?: number): Promise<SearchResult> {
    return await this.search('', {
   ...
   * 최근 기억 검색
   */
  async searchRecent(days: number = 7, limit?: number): Promise<SearchResult> {
   ...
   * 고정된 기억 검색
   */
  async searchPinned(limit?: number): Promise<SearchResult> {
    return await this.search('', {
      filters: { pinned: true },
      limit: limit || undefined
    });
   ...
   * 기억 중요도 설정
   */
  async setImportance(id: string, importance: number): Promise<MemoryItem> {
   ...
   * 기억 태그 추가
   */
  async addTags(id: string, tags: string[]): Promise<MemoryItem> {
   ...
   * 기억 태그 제거
   */
  async removeTags(id: string, tags: string[]): Promise<MemoryItem> {
    const memory = await this.get(id);
    if (!memory) {
      throw new Error(`Memory with id ${id} not found`);
    }
   ...
   * 기억 태그 업데이트
   */
  async setTags(id: string, tags: string[]): Promise<MemoryItem> {
   ...
   * 여러 기억 일괄 생성
   */
  async createBatch(params: CreateMemoryParams[]): Promise<MemoryItem[]> {
   ...
   * 여러 기억 일괄 삭제
   */
  async deleteBatch(ids: string[], hard: boolean = false): Promise<number> {
    let successCount = 0;
   ...
   * 여러 기억 일괄 고정/해제
   */
  async pinBatch(ids: string[], pin: boolean = true): Promise<number> {
    let successCount = 0;
