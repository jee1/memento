...
export class PerformanceMonitoringIntegration {
  ...
  private async performMonitoringCycle(): Promise<void> {
    try {
      ...
      if (metrics.search.averageSearchTime > 0) {
        this.alertService.checkPerformanceMetric(
          AlertType.RESPONSE_TIME,
          metrics.search.averageSearchTime,
          {
      ...
      if (metrics.search.cacheHitRate > 0) {
        this.alertService.checkPerformanceMetric(
          ...
          metrics.search.cacheHitRate * 100, // 백분율로 변환
          {
            component: 'cache',
            operation: 'search',
            totalSearches: metrics.search.totalSearches
  ...
  updateAlertThresholds(thresholds: Partial<MonitoringConfig['alertThresholds']>): void {
    if (thresholds.responseTime) {
      this.alertService.updateThreshold(
        AlertType.RESPONSE_TIME,
        AlertLevel.WARNING,
        { threshold: thresholds.responseTime.warning, operator: 'gt' }
      );
      this.alertService.updateThreshold(
        AlertType.RESPONSE_TIME,
        AlertLevel.CRITICAL,
        { threshold: thresholds.responseTime.critical, operator: 'gt' }
      );
    }
    ...
    if (thresholds.memoryUsage) {
      this.alertService.updateThreshold(
        AlertType.MEMORY_USAGE,
        AlertLevel.WARNING,
        { threshold: thresholds.memoryUsage.warning, operator: 'gt' }
      );
      this.alertService.updateThreshold(
        AlertType.MEMORY_USAGE,
        AlertLevel.CRITICAL,
        { threshold: thresholds.memoryUsage.critical, operator: 'gt' }
      );
    }
    ...
    if (thresholds.errorRate) {
      this.alertService.updateThreshold(
        AlertType.ERROR_RATE,
        AlertLevel.WARNING,
        { threshold: thresholds.errorRate.warning, operator: 'gt' }
      );
      this.alertService.updateThreshold(
        AlertType.ERROR_RATE,
        AlertLevel.CRITICAL,
        { threshold: thresholds.errorRate.critical, operator: 'gt' }
      );
    }
    ...
    if (thresholds.throughput) {
      this.alertService.updateThreshold(
        AlertType.THROUGHPUT,
        AlertLevel.WARNING,
        { threshold: thresholds.throughput.warning, operator: 'lt' }
      );
      this.alertService.updateThreshold(
        AlertType.THROUGHPUT,
        AlertLevel.CRITICAL,
        { threshold: thresholds.throughput.critical, operator: 'lt' }
      );
    }
