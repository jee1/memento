...
- [x] 4.0 Phase 1 테스트 작성 및 검증 (Phase 1)
...
- [x] 5.0 실패 감지 시스템 구현 (Phase 2)
  - [x] 5.1 FailureDetector 서비스 클래스 생성 (src/services/failure-detector.ts)
  - [x] 5.2 MCP Tool 호출 실패 감지 로직 구현 (ToolError, ValidationError, DatabaseError 등 에러 타입 감지)
  - [x] 5.3 BaseTool에 실패 감지 훅 통합 (handle 메서드에서 에러 발생 시 FailureDetector 호출)
  - [x] 5.4 사용자 피드백 기반 실패 감지 로직 구현 (키워드 기반 감지, 텍스트 메시지 분석)
  - [x] 5.5 성능 지표 미달 감지 로직 구현 (응답 시간 임계값, 정확도 지표, 에러율 임계값)
  - [x] 5.6 실패 이벤트 큐잉 시스템 구현 (AsyncTaskQueue 활용, 우선순위 기반 큐)
  - [x] 5.7 실패 이벤트 데이터 구조 정의 (tool_name, error_type, error_message, timestamp, context 등)

- [x] 6.0 Reflexion Worker 구현 (Phase 2)
  - [x] 6.1 ReflexionWorker 서비스 클래스 생성 (src/services/reflexion-worker.ts)
  - [x] 6.2 중복 감지 방지 로직 구현 (이벤트 키 생성: SHA256({tool_name}_{error_type}_{error_message_hash}), 슬라이딩 윈도우 5분)
  - [x] 6.3 재시도 및 백오프 로직 구현 (최대 3회 재시도, 지수 백오프: 1초, 2초, 4초)
  - [x] 6.4 동시성 제한 로직 구현 (최대 5개 동시 실행, 우선순위 기반 큐 처리)
  - [x] 6.5 큐 크기 제한 로직 구현 (최대 100개, 초과 시 FIFO로 가장 오래된 항목 제거)
  - [x] 6.6 장애 처리 로직 구현 (Worker 크래시 자동 재시작 최대 3회, 큐 적체 임계값 50개 경고)
  - [x] 6.7 auto_reflect 내부 함수 구현 (MCP Tool로 노출되지 않음, 실패 정보를 바탕으로 Reflexion 데이터 생성)
  - [x] 6.8 Reflexion 데이터 자동 생성 로직 구현 (failure_type 설정, 실패 설명 추출, 원래 작업 목표 추출, 템플릿 기반 개선 방안 제안)
  - [x] 6.9 BatchScheduler에 Reflexion Worker 통합 (백그라운드 실행, 메인 프로세스 격리)

- [x] 7.0 동일 작업 반복 실패 처리 로직 구현 (Phase 2)
  - [x] 7.1 동일 task_goal 감지 로직 구현 (Procedural Memory의 task_goal 필드 기준)
  - [x] 7.2 기존 reflection_notes 조회 로직 구현 (동일 task_goal에 대한 이전 Reflexion 기록 조회)
  - [x] 7.3 reflection_notes 업데이트 로직 구현 (공통 유틸리티 함수 사용: src/utils/reflection-notes-merge.ts, 1.5에서 구현된 함수 재사용)
  - [x] 7.4 배열 크기 제한 로직 구현 (공통 유틸리티 함수 사용: src/utils/reflection-notes-merge.ts, 1.5에서 구현된 함수 재사용)
  - [x] 7.5 반복 실패 패턴 분석 로직 구현 (동일 작업의 반복 실패 횟수 추적, 실패 패턴 분석)
  - [x] 7.6 ReflexionWorker에 반복 실패 처리 통합 (auto_reflect 함수 내에서 동일 task_goal 확인 및 공통 유틸리티 함수로 업데이트)

- [x] 8.0 Phase 2 테스트 작성 및 검증 (Phase 2)
  - [x] 8.1 FailureDetector 단위 테스트 작성 (MCP Tool 호출 실패 감지, 사용자 피드백 감지, 성능 지표 미달 감지)
  - [x] 8.2 ReflexionWorker 단위 테스트 작성 (중복 감지, 재시도 및 백오프, 동시성 제한, 큐 크기 제한)
  - [x] 8.3 동일 작업 반복 실패 처리 통합 테스트 작성 (task_goal 기반 감지, reflection_notes 업데이트, 배열 변환)
  - [x] 8.4 E2E 테스트 시나리오 작성 (Tool 호출 실패 → 자동 Reflexion 기록 → 동일 작업 재시도 시 개선 방안 적용)
  - [x] 8.5 성능 테스트 작성 (동시성, 큐 적체 시나리오, Worker 실패 복구)
  - [x] 8.6 측정 지표 수집 로직 구현 (실패 감지 재현율, 정밀도, Reflexion 기록 성공률)
