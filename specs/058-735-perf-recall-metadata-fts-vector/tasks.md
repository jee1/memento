...
**Input**: `specs/058-735-perf-recall-metadata-fts-vector/`
**Prerequisites**: spec.md, plan.md
**Branch**: `jee1/perf-recall-metadata-fts-vector`

## Phase 1: Setup

- [x] T001 Write `spec.md` / `plan.md` / `tasks.md` under `specs/058-735-perf-recall-metadata-fts-vector/`
...
- [x] T003 [US1] Add no-timer read-your-write test in `packages/memento-core/src/domains/memory/services/meta-memory-service.spec.ts` (`recordRecall` then `getStats`/`getStatsById` without 150ms wait)
...
- [x] T005 [US1] Overlay `statsBuffer` onto DB rows in `getStatsById` and `getStats` in `packages/memento-core/src/domains/memory/services/meta-memory-service.ts`
- [x] T006 [US1] Remove `setTimeout(..., 150)` from `getMetaStatsForResults` in `packages/memento-core/src/domains/memory/tools/recall-tool-envelope.ts`
...
- [x] T013 [US4] Record before/after recall p95 (local `recall_profile` or #737 `memento_prod` if available) in CHANGELOG Unreleased or `specs/058-735-perf-recall-metadata-fts-vector/p95-note.md`
