...
**Spec**: `specs/058-735-perf-recall-metadata-fts-vector/spec.md`
**Issue**: #735
**Parent Epic**: #733
...
| Module | Role |
|--------|------|
| `packages/memento-core/src/domains/memory/services/meta-memory-service.ts` | `getStats`/`getStatsById`가 `statsBuffer` overlay. debounce flush 유지. |
| `packages/memento-core/src/domains/memory/tools/recall-tool-envelope.ts` | `getMetaStatsForResults`에서 150ms sleep 삭제. |
