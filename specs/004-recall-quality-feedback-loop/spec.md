...
- benchmark Ground Truth가 없는 쿼리 카테고리에 CI 임계값이 설정된 경우: 데이터 부족 경고를 출력하고 해당 카테고리는 지표 계산에서 제외하되, blocking CI gate는 불완전한 benchmark로 실패.
...
- [ ] **SC-001**: 피드백 신호 도입 후 30일 이내에, 긍정 피드백을 받은 기억이 동일 쿼리 재호출 시 상위 5위 안에 재등장하는 비율이 도입 전 대비 10% 이상 향상된다. 측정 스크립트 구현 완료; 30일 운영 데이터 판정 대기.
- [x] **SC-002**: 쿼리 카테고리별 MRR 측정 결과에서 전체 평균과 1개 이상의 카테고리 간 MRR 차이가 0.1 이상 확인되어 취약 영역이 식별된다. 2026-08-13 category-report 기준 전체 가중 평균 0.7969, 카테고리 최대 차이 0.2969.
- [ ] **SC-003**: recall 호출 시 점수 구성 요소를 포함하는 옵션을 사용해도 응답 지연 **p95** 증가분이 100밀리초 이내다(측정 정의: 동일 벤치마크 워크로드·반복 실행). 단위 성능 검증 구현 완료; 동일 워크로드 p95 비교 측정 대기.
- [ ] **SC-004**: 피드백 이벤트 저장 실패 시에도 recall 응답은 정상 반환되며, 저장 실패율이 1% 미만으로 유지된다. 실패 격리·임계 판정 검증 구현 완료; 운영 실패율 판정 대기.
- [ ] **SC-005**: A/B 실험에서 MRR 차이가 0.05 이상인 프로파일 쌍에 대해 통계적 유의성(p < 0.05)이 검출된다. paired permutation 검증 구현 완료; 조건을 만족하는 프로파일 실험 결과 대기.
- [x] **SC-006**: 카테고리별 품질 리포트 단계(`npm run quality:benchmark:category-report`)에서, 벤치마크 DB **시드 완료 후** `collectCategoryMetrics` 실행부터 리포트 출력까지의 **스크립트 구간 벽시계**가 30초 이내다. CI 워크플로 전체(체크아웃·install·전체 테스트 등)의 총 벽시계 상한은 본 스펙의 자동 게이트 범위 밖이며, 운영에서 별도로 관리한다(FR-004·SC-003의 p95 지연과 구분). 2026-08-13 로컬 gate 통과.
