...
| # | 항목 | 처리 | 검증 |
|---|------|------|------|
| R-1 | `--path` 미동작 | **삭제.** 저장소 루트를 스크립트 위치에서 유도(`REPO_ROOT`)해 baseline 로드와 보고 경로가 항상 같은 기준을 쓴다. 대신 `--baseline=<path>` 추가 — 테스트 가능하고 실제로 동작한다 | 하위 디렉터리에서 실행해도 exit 0 |
| R-2 | 주석·문자열 오탐 | **줄 시작 앵커** `/^[ \t]*vi\.mock\(.../gm` | 스캔 57건·위반 8건 불변. 오탐 회귀 테스트 2개 추가 |
| R-3 | CLI 테스트 부재 | `execFileSync` 기반 **CLI 테스트 3개** 추가 (C1+C6 / C5 / 빈 baseline→위반 8건) | 게이트 스펙 10 → **17 tests** |
| R-4 | 중복 대입 15줄 | 블록 단위 중복 제거 | 잔여 중복 0, 36/36 유지 |
| R-5 | `isMain()` 재구현 | `lib/cli.ts` 의 `isMain(import.meta.url)` 사용 | — |
| R-6 | `package.json` 들여쓰기 | 4칸으로 교정 | — |
| R-7 | 계약서 58/48 오기 | 57/49 로 갱신 | — |
| R-8 | `existsSync` 가 디렉터리 통과 | `isFile()` 헬퍼로 파일만 인정 | 회귀 테스트 1개 추가 |
| R-9 | 없는 `--path` ENOENT | R-1 로 소멸 | — |
| R-10 | 깨진 심볼릭 링크에 게이트 사망 | `statSync(full, { throwIfNoEntry: false })` 가드 | — |
| R-11 | SC-004 문구 미달 | `afterEach` 에도 기준 상태 복원 추가. 이제 "테스트가 끝난 직후" 가 성립한다 | 36/36 유지 |
| R-12 | baseline 1건이 N건을 덮음 | **미해소.** 발생 확률이 낮고 수정하면 매칭 키에 카운트가 들어가 R-1 이 지적한 취약성이 재발한다. #826 에 기록 | — |
| R-13 | `vi.doMock`·템플릿 리터럴 사각지대 | 계약서 "판정" 절에 범위 한계 명시 + 코드 헤더 주석. 후속 **#826** | — |
| R-14 | FR-014 미강제(stale→exit 0) | 계약의 명시적 선택. `--strict` 검토를 #826 에 기록 | — |
| 기각 | `importOriginal` 승격 경로 | `research.md` R3 에서 **철회**하고 기각 사유 기록 | — |

**추가로 고친 것**: baseline 파싱 실패 시 스캔을 건너뛰고 종료하던 동작을 바꿔, 오류를 보고한 뒤 빈 baseline 으로 스캔을 계속한다(로컬 실행자가 결과를 보게). `--ci` 에서는 baseline 오류도 exit 1 로 차단한다.

**재검증**: `npm run lint` 0 errors · `npm run type-check` 통과 · `npm test` **474 files / 5104 passed / 1 skipped / 0 failed** (게이트 테스트 7개 증가) · 게이트 `57 스캔 / 위반 0 / baselined 8 / stale 0`.

**최종 판정: 머지 가능.** Important 4건 전부 해소. 미해소 R-12 는 #826 으로 추적하며 오늘 차단 게이트를 켜는 데 지장이 없다.
