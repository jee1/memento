...
function testSimpleAlerts() {
    ...
    try {
        ...
        // 1. 응답시간 알림 테스트
        console.log('\n📊 응답시간 알림 테스트:');
        const responseTimeAlerts = alertService.checkPerformanceMetric(AlertType.RESPONSE_TIME, 150, // WARNING 임계값(100ms) 초과
        { component: 'search_engine', operation: 'search' });
        console.log(`생성된 알림 수: ${responseTimeAlerts.length}`);
        // 2. 메모리 사용량 알림 테스트
        console.log('\n📊 메모리 사용량 알림 테스트:');
        const memoryAlerts = alertService.checkPerformanceMetric(AlertType.MEMORY_USAGE, 150, // WARNING 임계값(100MB) 초과
        { component: 'memory_manager', heapTotal: 200 });
        console.log(`생성된 알림 수: ${memoryAlerts.length}`);
        // 3. 에러율 알림 테스트
        console.log('\n📊 에러율 알림 테스트:');
        const errorRateAlerts = alertService.checkPerformanceMetric(AlertType.ERROR_RATE, 7, // WARNING 임계값(5%) 초과
...
// 직접 실행
testSimpleAlerts();
//# sourceMappingURL=test-simple-alerts.js.map