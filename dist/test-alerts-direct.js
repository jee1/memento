...
async function testAlertsDirect() {
    ...
    try {
        ...
        // 통합 모니터링 서비스 생성
        const monitoringIntegration = new PerformanceMonitoringIntegration(db, alertService, {
        ...
        const responseTimeAlerts = alertService.checkPerformanceMetric(AlertType.RESPONSE_TIME, 60, // WARNING 임계값(50ms) 초과
        ...
        const memoryAlerts = alertService.checkPerformanceMetric(AlertType.MEMORY_USAGE, 15, // WARNING 임계값(10MB) 초과
        { component: 'memory_manager', heapTotal: 100 });
        console.log(`생성된 알림 수: ${memoryAlerts.length}`);
        // 에러율 알림 테스트
        console.log('📊 에러율 알림 테스트 (3% - WARNING 임계값 초과):');
        const errorRateAlerts = alertService.checkPerformanceMetric(AlertType.ERROR_RATE, 3, // WARNING 임계값(1%) 초과
        ...
        if (activeAlerts.length > 0) {
            ...
            if (firstAlert) {
                ...
                const resolved = alertService.resolveAlert(firstAlert.id, 'test_user', '테스트용 해결');
                if (resolved) {
...
//# sourceMappingURL=test-alerts-direct.js.map