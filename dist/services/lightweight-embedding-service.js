...
export class LightweightEmbeddingService {
     ...
     * 키워드 가중치 적용
     */
    applyKeywordWeights(vector, words) {
        const weightedVector = [...vector];
        // 중요한 키워드 패턴에 가중치 적용
        for (const word of words) {
            if (!word || word.trim().length === 0)
                continue; // undefined 및 빈 문자열 체크
            const index = this.hashToIndex(word);
            // 배열 범위 체크
            if (index < 0 || index >= weightedVector.length)
                continue;
            // 안전한 배열 접근을 위한 체크
...
//# sourceMappingURL=lightweight-embedding-service.js.map