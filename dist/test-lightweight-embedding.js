...
async function testLightweightEmbeddingFunctionality() {
    ...
    try {
        ...
        for (const { query, description } of searchQueries) {
            ...
            try {
                ...
                results.forEach((result, index) => {
                    ...
                    const score = result.finalScore || result.score || 'N/A';
                    console.log(`       점수: ${typeof score === 'number' ? score.toFixed(3) : score}`);
                    if (result.textScore !== undefined && result.vectorScore !== undefined) {
                        console.log(`       텍스트: ${result.textScore.toFixed(3)}, 벡터: ${result.vectorScore.toFixed(3)}`);
                    }
                });
            }
            catch (error) {
        ...
        try {
            ...
            if (statsResult.length > 0 && statsResult[0].search_type === 'hybrid') {
                console.log('   ✅ 하이브리드 검색 활성화됨');
                console.log(`   📊 벡터 검색 사용 가능: ${statsResult[0].vector_search_available || false}`);
            }
        }
        catch (error) {
...
//# sourceMappingURL=test-lightweight-embedding.js.map