...
export class PinTool extends BaseTool {
    ...
    async handle(params, context) {
        const { id } = PinSchema.parse(params);
        // 데이터베이스 연결 확인
        this.validateDatabase(context);
        try {
            // 개선된 트랜잭션 재시도 로직 사용
            await DatabaseUtils.runTransaction(context.db, async () => {
                const result = await DatabaseUtils.run(context.db, 'UPDATE memory_item SET pinned = TRUE WHERE id = ?', [id]);
                if (result.changes === 0) {
...
//# sourceMappingURL=pin-tool.js.map