...
export class DatabaseUtils {
    ...
    static transactionStates = new WeakMap();
    /**
     * 트랜잭션 상태 확인
     */
    static getTransactionState(db) {
     ...
     * 트랜잭션 상태 설정
     */
    static setTransactionState(db, inTransaction, transactionId) {
        this.transactionStates.set(db, { inTransaction, transactionId });
    }
    /**
     * SQLite3 쿼리를 실행 (재시도 로직 포함)
     */
    static run(db, sql, params = [], maxRetries = 3) {
        let lastError = null;
     ...
     */
    static get(db, sql, params = [], maxRetries = 3) {
        let lastError = null;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
     ...
     */
    static all(db, sql, params = [], maxRetries = 3) {
        let lastError = null;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
     ...
     */
    static async runTransaction(db, transactionFn, maxRetries = 3) {
     ...
     * 트랜잭션 상태 확인
     */
    static isInTransaction(db) {
        return this.getTransactionState(db).inTransaction;
    }
    /**
     * 트랜잭션 강제 정리 (비상시 사용)
     */
    static forceCleanupTransaction(db) {
        try {
            // 트랜잭션 상태 강제 해제
            this.setTransactionState(db, false);
            // 롤백 시도
