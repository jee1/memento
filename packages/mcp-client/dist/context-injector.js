...
export class ContextInjector {
    ...
    async inject(query, options = {}) {
        ...
        const { maxResults = 10, tokenBudget = 1200, contextType = 'general', memoryTypes, recentDays, importanceThreshold = 0.3, pinnedOnly = false } = options;
        // 1. 관련 기억 검색
        ...
        // 2. 토큰 예산에 맞게 기억 압축
        const compressedMemories = await this.compressMemories(searchResult.items, tokenBudget);
        // 3. 컨텍스트 생성
        const context = this.generateContext(query, compressedMemories, contextType);
     ...
     * 기억 압축 (토큰 예산에 맞게)
     */
    async compressMemories(memories, tokenBudget) {
     ...
     * 컨텍스트 생성
     */
    generateContext(query, memories, contextType) {
        if (memories.length === 0) {
     ...
     * 컨텍스트 헤더 생성
     */
    getContextHeader(contextType) {
        const headers = {
            conversation: '💬 관련 대화 기록:',
            task: '📋 관련 작업 기록:',
            general: '🧠 관련 기억:'
        };
        return headers[contextType] || headers.general;
    }
    /**
     * 기억 포맷팅
     */
    formatMemories(memories) {
        return memories.map((memory, index) => {
            ...
            const tags = memory.tags && memory.tags.length > 0
                ? ` [${memory.tags.join(', ')}]`
                : '';
            const score = memory.score ? ` (관련도: ${(memory.score * 100).toFixed(1)}%)` : '';
     ...
     * 컨텍스트 푸터 생성
     */
    getContextFooter(query, memoryCount) {
        return `\n💡 위의 ${memoryCount}개 기억을 참고하여 "${query}"에 대해 답변해주세요.`;
    }
    /**
     * 빈 컨텍스트 생성
     */
    generateEmptyContext(query, contextType) {
        const messages = {
     ...
     * 대화 컨텍스트 주입
     */
    async injectConversationContext(query, tokenBudget = 1000) {
     ...
     * 작업 컨텍스트 주입
     */
    async injectTaskContext(query, projectId, tokenBudget = 1200) {
        const filters = {};
        if (projectId) {
     ...
     * 학습 컨텍스트 주입
     */
    async injectLearningContext(topic, tokenBudget = 1500) {
     ...
     * 프로젝트 컨텍스트 주입
     */
    async injectProjectContext(projectId, query, tokenBudget = 1000) {
