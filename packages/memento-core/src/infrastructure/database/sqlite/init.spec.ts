  ...
  describe('스키마 버전 검증', () => {
    ...
    it('Given: 빈 파일 DB, When: initializeDatabase로 초기화하면, Then: schema.sql 기준으로 모든 증분 마이그레이션 버전이 기록되어야 함', async () => {
      ...
      try {
        ...
        const auditTable = opened.prepare(
          "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'audit_log'"
        ).get();
        expect(auditTable).toBeDefined();
        for (const transport of ['mcp_stdio', 'mcp_http', 'mcp_ws', 'rest', 'http_admin']) {
          expect(() => opened!.prepare(`
            ...
            ) VALUES (?, ?, 'fresh-schema-probe', 'read', 'success', 'metadata_only', 1, 1, 'omitted', 'omitted', 'pass', ?)
          `).run(`fresh-${transport}`, transport, `fresh-hash-${transport}`)).not.toThrow();
        }
      } finally {
        if (opened) {
          try {
