...
const configuredSessions = new WeakSet<Database.Database>();

export function assertSqliteSessionConfigured(db: Database.Database): void {
  if (!configuredSessions.has(db)) {
    throw new Error('configureSqliteSession(db) must run before applySchema(db)');
  }
}
