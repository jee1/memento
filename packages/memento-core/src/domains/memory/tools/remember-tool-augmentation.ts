...
async function runEmbeddingAndNeighbors(
  ...
  try {
    embeddingResult = await embeddingServiceRef.createAndStoreEmbedding(dbRef, savedMemoryId, content, savedMemoryType as import('../../../shared/types/index.js').MemoryType);
  } catch (error) {
