...
export function insertMemoryItem(db: Database.Database, item: TestMemoryItem): void {
...
export function insertMemoryEmbedding(
  ...
  `, [embedding.memory_id, provider, embeddingJson, dim, dim]);
}
...
export function generateSampleMemoryItems(count: number = 10): TestMemoryItem[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const recallCount = i % 5;
    const hoursSinceAccess = (i % 3) * 24;
...
export function generateScenarioBasedTestData(count: number = 50, seed: number = 12345): TestMemoryItem[] {
  const items: TestMemoryItem[] = [];
  ...
  const scenarioCounts = {
    highVectorHighConsolidation: Math.floor(count * 0.2),
    highVectorLowConsolidation: Math.floor(count * 0.2),
    lowVectorHighConsolidation: Math.floor(count * 0.2),
    lowVectorLowConsolidation: Math.floor(count * 0.2),
    extreme: count - Math.floor(count * 0.8)
  };
  const scenarios = [
    { name: 'highVectorHighConsolidation', consolidationRange: [0.7, 0.95] as [number, number], n: scenarioCounts.highVectorHighConsolidation },
    { name: 'highVectorLowConsolidation', consolidationRange: [0.1, 0.3] as [number, number], n: scenarioCounts.highVectorLowConsolidation },
    { name: 'lowVectorHighConsolidation', consolidationRange: [0.7, 0.95] as [number, number], n: scenarioCounts.lowVectorHighConsolidation },
    { name: 'lowVectorLowConsolidation', consolidationRange: [0.1, 0.3] as [number, number], n: scenarioCounts.lowVectorLowConsolidation },
    { name: 'extreme', consolidationRange: [0.85, 0.99] as [number, number], n: scenarioCounts.extreme }
  ];
  let scenarioIndex = 0;
  scenarios.forEach(scenario => {
    for (let i = 0; i < scenario.n; i++) {
      ...
      const createdAt = new Date(now.getTime() - daysAgo * 24 * 60 * 60 * 1000);
      const consolidationScore = rng.randomFloat(scenario.consolidationRange[0], scenario.consolidationRange[1]);
      const recallCount = consolidationScore > 0.7 ? rng.randomInt(5, 20) : rng.randomInt(0, 4);
      const hoursSinceAccess = consolidationScore > 0.7 ? rng.randomInt(0, 24) : rng.randomInt(48, 168);
      const lastAccessedAt = new Date(now.getTime() - hoursSinceAccess * 60 * 60 * 1000);
      items.push({
...
export function generateSeededEmbeddings(
  ...
  return memoryIds.map((memoryId, index) => {
    ...
    if (vectorSimilarityRange) {
      ...
      embedding = Array(dimension).fill(0).map((_, i) => itemRng.randomFloat(-0.1, 0.1));
    }
    return { memory_id: memoryId, embedding, embedding_provider: 'tfidf', dim: dimension };
  });
}

export function generateSampleEmbeddings(memoryIds: string[], dimension: number = 1536): TestMemoryEmbedding[] {
  return memoryIds.map((memoryId, index) => {
    const embedding = Array(dimension).fill(0).map((_, i) => Math.sin((index * 10 + i) / dimension) * 0.1 + 0.1);
    return { memory_id: memoryId, embedding, embedding_provider: 'tfidf', dim: dimension };
  });
}
...
export function seedTestDatabase(
  ...
  const items = seed !== undefined ? generateScenarioBasedTestData(itemCount, seed) : generateSampleMemoryItems(itemCount);
  const memoryIds = items.map(item => item.id);
  items.forEach(item => insertMemoryItem(db, item));
  if (includeEmbeddings) {
    const embeddings = seed !== undefined ? generateSeededEmbeddings(memoryIds, 1536, seed) : generateSampleEmbeddings(memoryIds);
    embeddings.forEach(emb => insertMemoryEmbedding(db, emb));
  }
