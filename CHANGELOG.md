...
- **`048` 마이그레이션이 중복 본문 semantic 을 배포판 사용자의 DB 에서도 정리합니다** (#1139): #1137 이 만든 정리 스크립트 `scripts/repair-duplicate-semantic-content.ts` 는 **npm tarball 의 `files` 밖**이라 저장소를 clone 한 사용자에게만 닿았습니다. 결정 로직 `buildDuplicatePlan` 을 `@memento/core` 로 옮겨 마이그레이션과 스크립트가 **한 출처**를 공유하게 하고, 스크립트는 계획을 적용하는 일만 남겼습니다 — SQL·그룹 키·렌더 규칙은 그대로라 동작 변화는 없습니다. 기동 시 자동 적용되므로 중복 본문이 쌓인 기존 DB 도 업그레이드만으로 정리됩니다. 운영 DB 실측(수동 선적용분): 중복 그룹 228 → 16, 중복 행 1,566 → 96, 그중 triple 을 가진 행 1,449 → **0**, 소요 16초. 이미 정리된 DB 에서는 `{rerendered:0, deleted:0, passes:0}` 로 멱등하게 통과합니다. 048 은 임베딩을 다시 만들지 않습니다 — 재렌더된 본문의 stale 임베딩 갱신 수단은 #1155 에서 다룹니다.
...
- **기각 게이트가 WAF 차단 배치를 재시도합니다** (#1125): Cloudflare WAF 가 코드펜스 뒤의 `curl -s`·`curl -sL`·`wget -q` 가 든 요청 본문을 403 으로 막는데, 후보 1건만 걸려도 배치 10건 전체가 점수를 얻지 못해 그 질의가 fail-open 으로 통과했습니다. 이제 WAF 차단(본문 없는 403)일 때만 코드펜스 마커를 걷어 **1회** 재시도합니다. 1차 호출은 원문 그대로 보내므로 임계값 0.5 교정은 그대로입니다. 인증 403·500·타임아웃·네트워크 실패는 재시도하지 않습니다. 함께 `SEARCH_REJECTION_GATE_ON_ERROR=open|closed` 를 추가해 게이트가 점수를 하나도 얻지 못했을 때의 동작을 설정으로 고정할 수 있습니다 — **기본값은 `open`**(기각하지 않음)이며, `closed` 를 기본으로 두면 WAF 가 막는 본문을 가진 기억이 자기 자신을 찾는 질의에서 영구히 0건이 되기 때문입니다.

- **`MEMENTO_MCP_ERA` 가 컨테이너에 도달하지 못하던 문제** (#1129): #840 이 HTTP MCP dual-era 의 롤백 게이트로 만든 키인데 `docker-compose.base.yml` 의 env 화이트리스트에 없었습니다. `.env` 에 `legacy` 를 적어도 컨테이너 안에서는 계속 `dual` 이라 **롤백 수단이 존재하지 않았습니다.** 2026-09-23 에 실제로 이 상태였습니다 — 모던 era 가 SEP-2549 필수 캐시 필드를 빠뜨려 MCP 클라이언트가 죽었을 때 `legacy` 로 내려 회피할 수 없었고 코드 수정 PR 을 만들어야 했습니다. 이제 compose 가 호스트 값을 그대로 넘기고, 기본값은 코드와 같은 `dual` 입니다. `env.example` 에도 키를 문서화했습니다. #1115(`MEMENTO_API_TOKENS`)·#1095(기각 게이트)와 같은 부류이며, 가드 테스트(`tests/env-auth-vars-reach-compose.spec.ts`)에 세 번째 블록으로 추가했습니다. 운영 반영에는 `docker compose up -d --force-recreate` 가 필요합니다.

- **HTTP 모던 era 가 2026-07-28 의 필수 캐시 필드를 빠뜨리던 문제** (#1129): 서버가 `server/discover` 로 `2026-07-28` 을 맨 앞에 광고하면서 그 개정판을 실제로는 지키지 않았습니다. SEP-2549 는 cacheable result(`tools/list`·`prompts/list`·`resources/list`·`resources/templates/list`·`resources/read`·`server/discover`)에 `ttlMs`·`cacheScope` 를 **요구**하는데, stdio 는 `@modelcontextprotocol/server@2.0.0` 의 2026 코덱(`encodeResult`)이 채워 주는 반면 HTTP `/mcp` 는 JSON-RPC 응답을 손으로 만들어(`message-processor.ts`) 채우는 곳이 없었습니다. 클라이언트는 `connected · tools fetch failed` 로 멈추고 도구를 하나도 받지 못합니다 — `MEMENTO_MCP_ERA` 기본값이 `dual` 이라 기본 배포가 전부 이 경로를 탑니다. `applyModernSuccessEnvelope` 가 이제 `method` 를 받아 닫힌 목록에 속할 때만 두 필드를 채웁니다. 핸들러가 넣은 유효한 값이 이기고, 아니면 보수적 기본값 `{ ttlMs: 0, cacheScope: 'private' }` 입니다. 목록에 없는 메서드에는 붙이지 않습니다 — 목록이 닫혀 있어 규정에 없는 필드를 얹는 것도 위반이기 때문입니다. legacy era 와 stdio 는 바이트 단위로 그대로입니다. 알려진 곁가지: 회피책이었을 `MEMENTO_MCP_ERA=legacy` 는 이 키가 compose env 화이트리스트에 없어 컨테이너에 도달하지 않습니다.
