...
(function (global) {
  ...
  ns.renderSessions = function (sessions, append) {
    ...
    sessions.forEach(function (session) {
      ...
      ns.appendText(heading, 'strong', session.adapter_name || session.adapter || '알 수 없는 어댑터');
  ...
  ns.renderSessionDetail = function (session) {
    ...
    dom.addDefinition(list, '시작', ns.formatTime(session.started_at));
    dom.addDefinition(list, '마지막 이벤트', ns.formatTime(session.last_event_at || session.updated_at));
