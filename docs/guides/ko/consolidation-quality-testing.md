...
테스트 헬퍼(`packages/memento-core/src/test/helpers/consolidation-test-data.ts`)를 사용하면 자동으로 샘플 데이터를 생성할 수 있습니다:
...
npm test -- packages/memento-core/src/domains/search/algorithms/__tests__/search-ranking.spec.ts

# SearchResultCombiner 단위 테스트
npm test -- packages/memento-core/src/domains/search/algorithms/__tests__/search-result-combiner-consolidation.spec.ts
```
...
npm test -- packages/memento-core/src/domains/search/algorithms/__tests__/hybrid-search-engine-consolidation.spec.ts
```
...
```typescript
import { saveBaselineSnapshot } from '@memento/core/domains/monitoring/services/quality-assurance/vector-search-quality-metrics.js';
