...
데이터가 없으면 명령은 실패를 숨기지 않고 `artifacts/longmemeval-s/latest/`의 세 파일에 `dataset_missing`과 `not_run`을 기록한다. 정리 전 실행 증거는 불변 기준선의 [run-note.md](https://github.com/jee1/memento/blob/44ad88e2583b6486a30ca362729c68ebdeb45702/docs/_work/testing/longmemeval-s/latest/run-note.md)에 보존되어 있다.
...
정리 전 공개 산출물은 불변 기준선의 [run-note.md](https://github.com/jee1/memento/blob/44ad88e2583b6486a30ca362729c68ebdeb45702/docs/_work/testing/longmemeval-s/latest/run-note.md), [results.json](https://github.com/jee1/memento/blob/44ad88e2583b6486a30ca362729c68ebdeb45702/docs/_work/testing/longmemeval-s/latest/results.json), [judge-results.jsonl](https://github.com/jee1/memento/blob/44ad88e2583b6486a30ca362729c68ebdeb45702/docs/_work/testing/longmemeval-s/latest/judge-results.jsonl)에 보존되어 있다. Codex CLI 0.139.0의 `gpt-5.5`를 reader로, Claude Code 2.1.153의 `claude-sonnet-4-6`을 독립 judge로 사용했다. 6개 질문 유형에서 각 3건을 선택한 18건 표본의 accuracy는 55.56%, required evidence coverage는 62.50%였다.

이 실행은 외부 reader/judge 파이프라인 검증용이다. reader에는 oracle session을 제공했고 케이스당 12,000자 제한을 적용했으므로 Memento retrieval-to-answer 정확도로 해석하지 않는다. 표본, 비용, 토큰, truncation 영향과 개별 판정은 위의 불변 기준선 링크에 보존되어 있다.
