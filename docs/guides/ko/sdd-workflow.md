...
| 단계            | 산출물         | 위치                                                    | 설명                                            |
| ------------- | ----------- | ----------------------------------------------------- | --------------------------------------------- |
| **Design**    | 설계·브레인스토밍   | 기능별 디렉터리 내 `design.md` (아래 §2 참조). 또는 `docs/_work/design/`, `docs/_work/brainstorms/` | 요구사항 논의, 접근 방식, 제약 사항 정리.                     |
...
- **기능 디렉터리**: `docs/_work/plans/ko/YYYY-MM-DD-<기능명>/`
...
```
docs/_work/plans/ko/2026-03-11-memento-cli-for-ai/
...
**Design을 다른 경로에 둘 때**: 이슈만 있고 기능 디렉터리를 아직 만들지 않았다면 `docs/_work/design/` 또는 `docs/_work/brainstorms/`에 설계 문서를 먼저 둘 수 있다. SPEC/PLAN을 작성할 때 해당 기능 디렉터리(`docs/_work/plans/ko/YYYY-MM-DD-기능명/`)를 만들고, design 문서를 `design.md`로 복사·이동하거나 링크로 참조하면 된다.

**참고**: 과거에는 Design은 `docs/_work/design/`·`docs/_work/brainstorms/`, SPEC/PLAN은 `docs/specs/ko/`·`docs/_work/plans/ko/`에 나뉘어 있었으나, 앞으로는 위와 같이 **기능별 디렉터리에 design + spec + plan**을 함께 둔다. 기존 문서는 필요 시 이 구조로 옮기거나 링크만 통일할 수 있다.
...
### 5.1 이미 이슈/제안만 있는 경우 (예: docs/_work/issues/)

1. **기능 디렉터리 생성**: `docs/_work/plans/ko/YYYY-MM-DD-<기능명>/`를 만든다.
2. **Design 정리**: 위 디렉터리 안에 `design.md`를 두고 설계 초안을 쓴다. 이미 `docs/_work/design/`·`docs/_work/brainstorms/`에 문서가 있으면 해당 디렉터리로 복사·이동하거나 링크로 참조한다.
...
1. **기능 디렉터리** `docs/_work/plans/ko/YYYY-MM-DD-<기능명>/`를 만들고, **Design**을 `design.md`로 둔다(또는 기존 `_work/design/`·`_work/brainstorms/` 문서를 여기로 옮기거나 링크).
