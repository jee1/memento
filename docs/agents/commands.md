...
npm run docker:logs               # 컨테이너 로그 추적
npm run docker:down               # 컨테이너 삭제 — 아래 주의 참고
```
...
같은 트랜잭션에서 갱신하므로 키워드 검색은 정확하지만, **벡터 임베딩은 바뀌기 전 본문 기준으로
남습니다** — 그 행은 옛 본문으로 검색에 걸립니다.

저장소 체크아웃에서는 이렇게 갱신합니다:
