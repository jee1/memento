...
**주의**: 실행용 DDL의 진실 공급원은 `packages/memento-core/src/infrastructure/database/sqlite/schema.sql`과 마이그레이션 스크립트이며, 본 문서는 그에 대한 설명·정리용이다.
**연관**: [마이그레이션 시스템 가이드](../../guides/ko/migration-system-guide.md), [전체 테이블 ERD](database-erd.md).
...
- **관련 문서**:
  ...
  - 스키마 DDL: `packages/memento-core/src/infrastructure/database/sqlite/schema.sql`
  - 마이그레이션: `packages/memento-core/src/infrastructure/database/sqlite/migration/migrations/`
...
상세는 `packages/memento-core/src/infrastructure/database/sqlite/migration/migrations/` 내 해당 `.ts` 파일 참조.
