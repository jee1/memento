...
관리 대시보드와 앵커 맵은 `static/` 아래 순수 CSS로 구성되어 있습니다. 새 화면을 추가할 때마다 색·간격을 하드코딩하면 다크 그래프 뷰와 라이트 대시보드가 어긋나므로, **디자인 토큰(`tokens.css`)과 공통 컴포넌트(`components.css`)** 를 통해서만 스타일을 참조합니다. 이 문서는 그 규약과 주요 토큰·클래스를 정리합니다.
