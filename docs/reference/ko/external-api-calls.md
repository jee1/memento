...
```bash
# 핵심 모듈만 검색
npx tsx scripts/find-external-api-calls.ts --core-only --format=json > docs/reference/ko/external-api-calls.json

# 전체 검색
npx tsx scripts/find-external-api-calls.ts --format=json > docs/reference/ko/external-api-calls-full.json
```
