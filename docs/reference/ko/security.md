...
- **브라우저 비밀 처리**: 서버는 브라우저 자산에 `ADMIN_API_KEY`를 전달하지 않습니다. 운영자는 `/auth/session`으로 로그인하고, 서버는 입력된 키를 HTTP-only 세션 쿠키로 교환합니다. `/dashboard`, `/graph` 같은 정적 페이지는 이 쿠키만 사용하며 키를 JavaScript로 부트스트랩하지 않습니다.
