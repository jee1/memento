...
- **Upstream-blocked (accepted risk, no force-override)** — 2026-09-16 재측정:

| Package path | Advisory / notes | Why blocked | Tracking |
|--------------|------------------|-------------|----------|
| (2026-09-16 기준 수용 중인 upstream-blocked 항목 없음 — #989) | — | — | — |

- **High 8건 처리 대장 (#909)** — 2026-09-16 측정 (`npm audit`: high 0 / moderate 3, `--omit=dev`: high 0):

| 패키지 (경로) | 취약점 ID | 조치 또는 수용 사유 | 재검토 조건 |
|---------------|-----------|---------------------|-------------|
| `@huggingface/transformers` (direct, prod) | 집계 노드 — `onnxruntime-node` · `sharp` 경유 | **조치 완료 (검증됨)** — lockfile `4.3.0` (`^4.2.0` 범위 내 bump, #989) | 신규 advisory 발생 시 |
| `adm-zip` ← `onnxruntime-node` ← `@huggingface/transformers` (prod) | [GHSA-xcpc-8h2w-3j85](https://github.com/advisories/GHSA-xcpc-8h2w-3j85) (High), [GHSA-vwc7-r8mq-g2x9](https://github.com/advisories/GHSA-vwc7-r8mq-g2x9) (Moderate) | **조치 완료 (검증됨)** — 전이 `adm-zip@0.6.1` via `onnxruntime-node@1.30.0` (#989) | 신규 advisory 발생 시 |
| `onnxruntime-node` ← `@huggingface/transformers` (prod) | `adm-zip` 경유 (자체 advisory 없음) | **조치 완료 (검증됨)** — lockfile `onnxruntime-node@1.30.0` via `@huggingface/transformers@4.3.0` (#989) | 신규 advisory 발생 시 |
| `sharp` (direct + ← `@huggingface/transformers`, prod) | [GHSA-f88m-g3jw-g9cj](https://github.com/advisories/GHSA-f88m-g3jw-g9cj) (libvips, `sharp>=0.35` 필요), [GHSA-rgj7-g3m4-5g8c](https://github.com/advisories/GHSA-rgj7-g3m4-5g8c) (libheif, `sharp>=0.35.4` 필요) | **조치 완료 (검증됨)** — `package.json` `^0.34.4→^0.35.4`, lockfile `0.35.4` (#989) | sharp major 발생 시 |
