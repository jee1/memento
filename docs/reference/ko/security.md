...
- **브라우저 비밀 처리**: 서버는 브라우저 자산에 `ADMIN_API_KEY`를 전달하지 않습니다. 운영자는 `/auth/session`으로 로그인하고, 서버는 입력된 키를 HTTP-only 세션 쿠키로 교환합니다. `/dashboard`가 권장 진입점이며, `/graph`를 직접 열어도 같은 세션 모델로 로그인/재인증할 수 있습니다. 두 정적 페이지 모두 키를 JavaScript로 부트스트랩하지 않습니다.
