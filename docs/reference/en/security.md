...
- **Upstream-blocked (accepted risk, no force-override)** — remeasured 2026-09-16:

| Package path | Advisory / notes | Why blocked | Tracking |
|--------------|------------------|-------------|----------|
| (No upstream-blocked items accepted as of 2026-09-16 — #989) | — | — | — |

- **High-severity ledger, 8 findings (#909)** — measured 2026-09-16 (`npm audit`: 0 high / 3 moderate; `--omit=dev`: 0 high):

| Package (path) | Advisory | Action or acceptance rationale | Re-review trigger |
|----------------|----------|--------------------------------|-------------------|
| `@huggingface/transformers` (direct, prod) | Aggregate node — via `onnxruntime-node` and `sharp` | **Fixed (verified)** — lockfile `4.3.0` (wanted bump within `^4.2.0`, #989) | On new advisories |
| `adm-zip` ← `onnxruntime-node` ← `@huggingface/transformers` (prod) | [GHSA-xcpc-8h2w-3j85](https://github.com/advisories/GHSA-xcpc-8h2w-3j85) (High), [GHSA-vwc7-r8mq-g2x9](https://github.com/advisories/GHSA-vwc7-r8mq-g2x9) (Moderate) | **Fixed (verified)** — transitive `adm-zip@0.6.1` via `onnxruntime-node@1.30.0` (#989) | On new advisories |
| `onnxruntime-node` ← `@huggingface/transformers` (prod) | Via `adm-zip` (no advisory of its own) | **Fixed (verified)** — lockfile `onnxruntime-node@1.30.0` via `@huggingface/transformers@4.3.0` (#989) | On new advisories |
| `sharp` (← `@huggingface/transformers`, prod) | [GHSA-f88m-g3jw-g9cj](https://github.com/advisories/GHSA-f88m-g3jw-g9cj) (libvips, needs `sharp>=0.35`), [GHSA-rgj7-g3m4-5g8c](https://github.com/advisories/GHSA-rgj7-g3m4-5g8c) (libheif, needs `sharp>=0.35.4`) | **Fixed (verified)** — lockfile `0.35.4` via `@huggingface/transformers@4.3.0` (#989); removed root direct dependency (#993) | On sharp major bumps |
