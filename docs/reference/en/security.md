...
- **Browser secret handling**: The server does **not** deliver `ADMIN_API_KEY` to browser assets. Operators sign in through `/auth/session`, which exchanges the typed key for an HTTP-only session cookie. Static pages such as `/dashboard` and `/graph` rely on that cookie and never bootstrap the key into JavaScript.
