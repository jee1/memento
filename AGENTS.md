...
## Active Technologies
- TypeScript 5.x, Node.js 20+ (문서 및 설정 파일 중심 변경) + npm workspaces, existing memento server/core/client packages (신규 의존성 없음) (016-env-config-cleanup)
- N/A (환경설정 파일과 문서 변경 중심) (016-env-config-cleanup)

## Recent Changes
- 016-env-config-cleanup: Added TypeScript 5.x, Node.js 20+ (문서 및 설정 파일 중심 변경) + npm workspaces, existing memento server/core/client packages (신규 의존성 없음)
