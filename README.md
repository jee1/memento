...
LLM은 대화가 끝나면 모든 것을 잊는다. 이름도, 결정도, 지난주에 쌓인 맥락도. 이건 모델의 한계가 아니라 **기억 인프라의 부재**다.

Memento는 그 인프라다. 단순 저장소가 아니라, 기억이 생성·분류·강화·망각되는 **MCP 기반 기억 운영 체제**.

작업기억·일화기억·의미기억·절차기억을 `remember`의 `type`으로 구분하고, 쓰인 기억은 강화되며, 쓸모없어진 기억은 망각 정책으로 정리된다. 비슷한 기억은 벡터로 연결되고, 핵심 맥락은 앵커로 다음 대화에 바로 복원된다.

모노레포·패키지 구조·빌드 명령은 [AGENTS.md](AGENTS.md)를 보세요.

## 빠른 시작

패키지 매니저는 **npm**만 지원합니다 (`pnpm`/`yarn` 미지원).
...
기억 DB는 `${CLAUDE_PLUGIN_DATA}/memory.db`에 두어 플러그인 업데이트 후에도 유지됩니다. `/plugin` 패널에서 `memento` MCP가 연결됐는지 확인하세요.
...
설정 파일 위치: Cursor는 `.cursor/mcp.json` 또는 `~/.cursor/mcp.json`, Claude Desktop·Claude Code는 [INSTALL.md](INSTALL.md)와 [Cursor MCP 설정 가이드](docs/guides/ko/cursor-mcp-setup.md)를 참고하세요.
...
Docker, 소스 빌드, HTTP MCP(다중 에이전트), 환경 오버레이는 [INSTALL.md](INSTALL.md)를 따르세요.

## 기본으로 보이는 도구

서버에는 도구 22개가 등록되어 있지만, `tools/list`에는 기본적으로 **`recall` · `remember` · `memory_injection` · `feedback` 4개만** 노출됩니다(v1.18+). 나머지는 등록된 채 호출은 가능하고, 목록에서만 빠집니다. 전부 나열하려면 `MEMENTO_TOOLSET=full`.
...
더 많은 연결 방식(MCP SDK, `@jee1/memento-client`, 외부 비서 SDK)은 [docs/README.md](docs/README.md)와 [docs/integrations/](docs/integrations/README.md)를 보세요.
...
기여는 [CONTRIBUTING.md](CONTRIBUTING.md)를 따르세요. 라이선스는 [MIT](LICENSE)입니다.
